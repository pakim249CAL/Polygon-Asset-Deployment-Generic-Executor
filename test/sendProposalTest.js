...
describe("Send Proposal", function() {
    ...
    before(async function() {
        ...
        function addReserveFactor(tokenAddress, reserveFactor) {
            let encodedArguments = ethers.utils.defaultAbiCoder.encode(['address', 'uint256'], [tokenAddress, reserveFactor]);
            proposalActions.targets.push(polygonLendingPoolConfiguratorAddress);
            proposalActions.values.push(ethers.BigNumber.from('0'));
        ...
        function addConfigureReserveAsCollateralAction(tokenAddress, ltv, lt, lb) {
            let encodedArguments = ethers.utils.defaultAbiCoder.encode(['address', 'uint256', 'uint256', 'uint256'], [tokenAddress, ltv, lt, lb]);
            proposalActions.targets.push(polygonLendingPoolConfiguratorAddress);
            proposalActions.values.push(ethers.BigNumber.from('0'));
        ...
        addConfigureReserveAsCollateralAction(ghstAddress, ethers.BigNumber.from('2500'), ethers.BigNumber.from('4500'), ethers.BigNumber.from('11250'));
        addConfigureReserveAsCollateralAction(balAddress, ethers.BigNumber.from('2000'), ethers.BigNumber.from('4500'), ethers.BigNumber.from('11000'));
        addConfigureReserveAsCollateralAction(dpiAddress, ethers.BigNumber.from('2000'), ethers.BigNumber.from('4500'), ethers.BigNumber.from('11000'));
        addConfigureReserveAsCollateralAction(crvAddress, ethers.BigNumber.from('2000'), ethers.BigNumber.from('4500'), ethers.BigNumber.from('11000'));
        addConfigureReserveAsCollateralAction(sushiAddress, ethers.BigNumber.from('2000'), ethers.BigNumber.from('4500'), ethers.BigNumber.from('11000'));
        addConfigureReserveAsCollateralAction(linkAddress, ethers.BigNumber.from('5000'), ethers.BigNumber.from('6500'), ethers.BigNumber.from('10750'));
        addConfigureReserveAsCollateralAction(maticAddress, ethers.BigNumber.from('6500'), ethers.BigNumber.from('7000'), ethers.BigNumber.from('11000'));
        ...
        */
        ...
        // This outputs to a file the data output from the statesender
        // Just a timesaver for debugging
        fxRootEncodedRootCalldata = ethers.utils.defaultAbiCoder.encode(
            ['address', 'address', 'bytes'],
            ['0xEE56e2B3D491590B5b31738cC34d5232F378a8D5', polygonBridgeExecutorAddress, proposalActions.encodedActions]
        );
        fs.writeFile('FxData.txt', fxRootEncodedRootCalldata, (err) => {
            if (err) throw err;
            console.log('Safe to quit test if you just want the output data');
        });

        //This impersonates accounts with enough AAVE to make a proposal and pass them.
    ...
    describe("Send Proposal", function() {
        ...
        it("Should be able to send proposal (will fail if not on ETH network)", async function() {
            // Make the proposal
